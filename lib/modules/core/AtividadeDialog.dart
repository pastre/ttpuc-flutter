...
class AtividadeState extends State<AtividadeWidget>{
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(backgroundColor: PUC_COLOR, title: Text('Adicione uma atividade'),),
        body: Column(children: buildOptions(),),
  ...
  List<Widget> buildOptions() {
    ...
    ret.add(buildOption(
        TextField(
            ...
            decoration: this.decorateTextField('Nome', 'Ex.:Prova, TDE, Atividade, etc...'),
    ...
    ret.add(buildOption(
        TextField(
          ...
          decoration: this.decorateTextField('Descrição', 'Ex.: Prova valendo 10 pontos. Estudar capítulos 2 e 3 do livro'),
          cursorColor: PUC_COLOR,
        )));
    ret.add(SizedBox(height: 32.0,));
    ...
    ret.add(buildOption(SimpleDialogOption(child: Row(children: <Widget>[Text('Pronto', style: TextStyle(color: PUC_COLOR),)], crossAxisAlignment: CrossAxisAlignment.end, mainAxisAlignment: MainAxisAlignment.end,), onPressed: (){print('Pressed pronto');},)));
//    ret.add(SizedBox(height: 8.0,));
  ...
  InputDecoration decorateTextField(String name, String helper){
    return InputDecoration(
      ...
      helperText: helper,
      helperStyle: TextStyle(color: PUC_COLOR.withAlpha(150), fontStyle: FontStyle.italic),
      enabledBorder: UnderlineInputBorder(borderSide: BorderSide(color: Colors.grey)),
      focusedBorder: UnderlineInputBorder(borderSide: BorderSide(color: PUC_COLOR),),
...
//    meses = ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'];
    ...
    for(int mes = startTime.month; mes <= 12; mes++){
      ...
      for(int dia = 1; dia <=days; dia++){
        ...
        ret.add(Text('${weekDays[dateTime.weekday - 1]}, ${dateTime.day} de ${meses[dateTime.month - 1]}'));
      }
    }
